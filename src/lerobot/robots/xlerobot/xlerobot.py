...
class XLerobot(Robot):
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.config.max_relative_target is not None:
            ...
            # Combine all present positions, suffixing keys with ".pos" so they match
            # the action keys (e.g. "left_arm_shoulder_lift.pos") looked up just below.
            present_pos = {
                f"{name}.pos": pos
                for name, pos in {**present_pos_left, **present_pos_right, **present_pos_head}.items()
            }
